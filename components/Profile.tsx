...
import { ArrowLeft, Smartphone, Globe2, Sparkles, Moon, Sun, KeyRound, Shield } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="glass-panel rounded-3xl p-5 shadow-xl border border-white/60 dark:border-white/10 bg-white/80 dark:bg-slate-900/70 backdrop-blur-xl space-y-4">
        ...
        <div className="rounded-2xl border border-slate-100 dark:border-slate-800 p-4 bg-white/70 dark:bg-slate-800/60 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${isDark ? 'bg-slate-800 text-amber-300' : 'bg-indigo-50 text-indigo-600'}`}>
              {isDark ? <Moon size={18} /> : <Sun size={18} />}
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">Theme</p>
              <p className="text-lg font-semibold text-slate-900 dark:text-white">{isDark ? 'Dark' : 'Light'}</p>
              <p className="text-sm text-slate-500 dark:text-slate-400">Switch to match your mood</p>
            </div>
          </div>
          <button
            onClick={toggleTheme}
            disabled={!profile}
            className="inline-flex items-center justify-center px-4 py-2 rounded-full bg-indigo-600 text-white font-semibold shadow-md shadow-indigo-500/30 hover:bg-indigo-500 transition-colors active:scale-95 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            Toggle Theme
          </button>
        </div>
