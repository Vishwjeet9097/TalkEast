...
export class StorageService {
    ...
    this.initPromise = new Promise((resolve, reject) => {
      try {
        ...
        request.onerror = (event) => {
          ...
          reject(
            new Error(
              `Database error: ${error?.message || "Failed to open database"}`
            )
          );
        };
            ...
            reject(
              new Error(
                `Database initialization failed: ${
                  seedError.message || "Unknown error"
                }`
              )
            );
          }
        ...
        request.onupgradeneeded = (event) => {
          try {
            ...
            if (!db.objectStoreNames.contains("job_batches")) {
              ...
              const store = db.createObjectStore("job_batches", {
                keyPath: "id",
              });
            ...
            if (!db.objectStoreNames.contains("practiceHistory")) {
              const store = db.createObjectStore("practiceHistory", {
                keyPath: "id",
              });
            ...
            reject(
              new Error(
                `Database upgrade failed: ${
                  upgradeError.message || "Unknown error"
                }`
              )
            );
          }
  ...
  async getCourses(): Promise<Course[]> {
    ...
    for (const course of courses) {
      try {
        ...
        console.log(
          `Course "${course.title}": ${course.chapters.length} chapters`
        );
      } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        ...
        request.onsuccess = () => {
          ...
          console.log(
            `getChaptersForCourse(${courseId}): Found ${chapters.length} chapters`
          );
          resolve(chapters);
        };
  ...
  async getStats(): Promise<UserStats> {
    ...
    const existing = (await this.get("userStats", "current")) as
      | UserStats
      | undefined;
    ...
    const stats: UserStats = existing
      ? {
          ...existing,
          todayHighScore:
            existing.lastReviewDate === today ? existing.todayHighScore : 0,
          dailyReviewCompleted:
            existing.lastReviewDate === today
              ? existing.dailyReviewCompleted
              : false,
